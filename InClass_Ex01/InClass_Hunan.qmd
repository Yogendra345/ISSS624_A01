---
title: "InClass_Ex01 : Spatial Weights and Applications"
...
-   Import csv file using appropriate function of [**readr**](https://www.rdocumentation.org/packages/readr/versions/2.1.3) package
...
```{r}
hunan <- st_read(dsn = "Hunan_data/geospatial", 
                 layer = "Hunan")
```
...
```{r}
hunan2012 <- read_csv("Hunan_data/aspatial/Hunan_2012.csv")
```
...
tmap_arrange(basemap, gdppc, widths = 1, asp=0, ncol=2,outer.margins = 0.01)
```
...
tmap_arrange(gdppc, lag_gdppc, asp=0.6, ncol=2)
```
...
tmap_arrange(gdppc, lag_sum_gdppc, asp=0.65, ncol=2)
```
...
tmap_arrange(gdppc, w_avg_gdppc, asp=0.65, ncol=2)
```
...
tmap_arrange(gdppc, w_sum_gdppc, asp=0.65, ncol=2)
```
